...
let round_dfrac x = 
  let m = 100.0 in  (* 10^2 for 2 decimal places *)
  Float.round (x *. m) /. m
...
and eval_bop bop e1 e2 = 
  ...
  | FAdd, Float a, Float b -> Float (round_dfrac (a +. b))
  | FMult, Float a, Float b -> Float (round_dfrac (a *. b))
  | FSub, Float a, Float b -> Float (round_dfrac (a -. b))
  | FDiv, Float a, Float b -> 
    if b = 0.0 then failwith "Division by zero"
    else Float (round_dfrac (a /. b))
